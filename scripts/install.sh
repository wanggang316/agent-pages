...
set -euo pipefail
#
# install.sh — make this machine capable of /build-h5.
#
...
#   ./scripts/install.sh [--gallery PATH] [--site URL] [--name "HTML <Pages/>"] [--with-hook] [--force]
...
say() { printf '  %s\n' "$*"; }
echo "build-h5 install"

default_gallery_name="HTML <Pages/>"
...
if [ -f "$cfg_file" ] && [ "$force" -ne 1 ]; then
  ...
  cat > "$cfg_file" <<EOF
# build-h5 configuration (generated by install.sh; edit freely)
...
site["title"] = title
site.setdefault("description", "Pages generated from conversations.")
