...
[ -n "$project" ] || bh5_die "--project is required (default to the basename of your Claude Code cwd)"
[ -n "$slug" ]    || bh5_die "--slug is required (kebab-case, short, English)"

bh5_require_gallery
gallery="$BUILD_H5_GALLERY_PATH"

# --- sync ---
git -C "$gallery" checkout "$BUILD_H5_BRANCH" >/dev/null 2>&1 || true
if [ "$do_pull" -eq 1 ]; then
  if ! git -C "$gallery" pull --rebase origin "$BUILD_H5_BRANCH" >/dev/null 2>&1; then
...
printf '{"galleryPath":"%s","project":"%s","projectDir":"%s","date":"%s","dateHuman":"%s","slug":"%s","fileName":"%s","targetPath":"%s","relPath":"%s","isNewProject":%s,"collisionResolved":%s}\n' \
  "$(bh5_json_escape "$gallery")" \
  "$(bh5_json_escape "$project")" \
  "$(bh5_json_escape "$project_dir")" \
  "$date" "$date_human" \
  "$(bh5_json_escape "$slug")" \
  "$(bh5_json_escape "$base")" \
  "$(bh5_json_escape "$target")" \
  "$(bh5_json_escape "$rel")" \
