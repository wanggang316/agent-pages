...
#   1. AGENT_PAGES_GALLERY_PATH from ~/.claude/agent-pages/config.env (written by install.sh)
...
: "${AGENT_PAGES_GALLERY_PATH:=${BUILD_H5_GALLERY_PATH:-$_ap_repo_root}}"
...
: "${AGENT_PAGES_GALLERY_NAME:=${BUILD_H5_GALLERY_NAME:-Agent <Pages/>}}"
: "${AGENT_PAGES_DEFAULT_PROJECT:=${BUILD_H5_DEFAULT_PROJECT:-}}"

# expand a leading ~ / $HOME in the gallery path
AGENT_PAGES_GALLERY_PATH="${AGENT_PAGES_GALLERY_PATH/#\~/$HOME}"
AGENT_PAGES_GALLERY_PATH="${AGENT_PAGES_GALLERY_PATH/#\$HOME/$HOME}"

ap_die() { printf 'agent-pages: %s\n' "$*" >&2; exit 1; }

ap_require_gallery() {
  [ -n "$AGENT_PAGES_GALLERY_PATH" ] || ap_die "gallery path is empty; run scripts/install.sh or set AGENT_PAGES_GALLERY_PATH"
  [ -d "$AGENT_PAGES_GALLERY_PATH/.git" ] || ap_die "gallery is not a git repo: $AGENT_PAGES_GALLERY_PATH"
  [ -f "$AGENT_PAGES_GALLERY_PATH/index.html" ] || ap_die "no index.html in gallery: $AGENT_PAGES_GALLERY_PATH"
}
