...
- **发布只用 publish.sh**：不要在画廊里 `git add -A`；脚本只会 stage 页面 + `index.html` + `data.json`。

## 配置与脚本在哪

- 运行时配置：`config.env`（`AGENT_PAGES_PATH` / `AGENT_PAGES_REPO` / `AGENT_PAGES_BRANCH` / `AGENT_PAGES_SITE_BASE_URL` / `AGENT_PAGES_NAME`）。由 `install.sh` 写入，可手改。
- 确定性脚本在画廊里：`$AGENT_PAGES_PATH/scripts/`（`new-page.sh` 同步+算路径、`publish.sh` 登记+commit+push+打开、`sync-upstream.sh` 拉模板更新）。
