...
name: agent-pages
description: |
  把当前会话中的主题/资料生成为一个独立可浏览的 HTML 页面（结构清晰、化繁为简、由浅入深，强调图形/表格/动效与精致的 UI/UE），落地到你的画廊仓库并自动 commit + push + 在浏览器中打开。常规入口是 `/agent-pages <主题或说明>`；插件注入的 `use-agent-pages` 引导纲要会在长方案场景提示用户可以使用 `/agent-pages`，确认用户希望生成页面后再进入写文件、发布和提交流程。
---
...
agent-pages 作为插件安装后，其 SessionStart hook 会注入 `use-agent-pages` 引导纲要：在用户请求详细设计方案 / 技术方案 / 架构设计、且做成自包含 HTML 工件更清楚时，建议用户考虑 `/agent-pages <主题>`；这只是推荐入口，后续是否生成页面以用户确认或明确命令为准。
